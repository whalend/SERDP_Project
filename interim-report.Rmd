...
![Figure XX. Burn unit map example](fort gordon)
...
litter_moisture_fig <- ggplot(filter(biomass_plot_summary, !is.na(years_since_fire)), 
      ...
      geom_smooth(aes(fill = imcy_inv), se = T, alpha = .2, method = "lm", show.legend = F) +
            geom_point(aes(color = imcy_inv), size = 2, position = position_jitterdodge()) +
      invasion_color +
      invasion_fill +
...
Dung surveys indicated that white-tailed deer and eastern cottontail rabbit were typically the most abundant host species at each installation (Figure 12). 
...
**Figure 12.** Evidence of host species densities at each installation. Note that the y-axis has different scales in each panel. Zeroes (no bars) are installations that weren't visited during that year.

```{r dung density vs time since fire}
dung_species <- c("deer","cottontail", "turkey")
