...
sem_data <- sem_data %>%
  mutate(biomass_log = log(avg_dry_standing_gm2),
         ...
         prev_human_no_Ramb_logit = car::logit(prev_human_no_Ramb),
         prev_human_no_Ramb_beta = case_when(
           prev_human_no_Ramb==0 ~ prev_human_no_Ramb+.01,
           prev_human_no_Ramb==1 ~ prev_human_no_Ramb-.01, 
           TRUE ~ prev_human_no_Ramb), 
         prev_human_no_Ramb_log = log(prev_human_no_Ramb+.01),
         
         prev_human_logit = car::logit(prev_human),
         prev_human_log = log(prev_human+.01),
         
         prev_human_animal_logit = car::logit(prev_human_animal),
         prev_human_animal_log = log(prev_human_animal+.01)
  )
...
human_no_Ramb_mod_allHosts <- glmer(
  cbind(ticks_human_no_Ramb, tick_abundance) ~ ticks_per_trap + total_clusters1m + (1 | inst_name/plot_id), 
  data = sem_data, 
...
# 
# m1 <- lmer(prev_human_no_Ramb_log ~  ticks_per_trap + total_clusters1m + (1 | inst_name/plot_id), 
#            data = sem_data)
...
human_no_Ramb_mod_noRabbit <- glmer(
  cbind(ticks_human_no_Ramb, tick_abundance) ~ ticks_per_trap + deer_other + (1 | inst_name/plot_id), 
...
human_no_Ramb_mod_deer <- glmer(
  cbind(ticks_human_no_Ramb, tick_abundance) ~ ticks_per_trap + deer + (1 | inst_name/plot_id), 
  data = sem_data, family = binomial(link = "logit"),
  glmerControl(optimizer = "Nelder_Mead", optCtrl=list(maxfun=1e6)))
# summary(human_no_Ramb_mod_deer)

human_no_Ramb_mod_noHosts <- glmer(
  cbind(ticks_human_no_Ramb, tick_abundance) ~ ticks_per_trap + (1 | inst_name/plot_id), 
  data = sem_data, family = binomial(link = "logit"),
  glmerControl(optimizer = "Nelder_Mead", optCtrl=list(maxfun=1e6)))
# summary(human_no_Ramb_mod_noHosts)

a <- AIC(human_no_Ramb_mod_allHosts, human_no_Ramb_mod_noRabbit, human_no_Ramb_mod_deer, human_no_Ramb_mod_noHosts)
## no difference between model AIC
...
# 
# tab_model(human_no_Ramb_mod_allHosts, human_no_Ramb_mod_deer, 
# human_no_Ramb_mod_noRabbit, human_no_Ramb_mod_noHosts)
...
human_mod_allHosts <- glmer(
  cbind(ticks_human, tick_abundance) ~ ticks_per_trap + total_clusters1m + (1 | inst_name/plot_id), 
  data = sem_data, family = binomial(link = "logit"),
  glmerControl(optimizer = "Nelder_Mead", optCtrl=list(maxfun=1e6)))
# summary(human_mod_allHosts)

human_mod_noRabbit <- glmer(
  cbind(ticks_human, tick_abundance) ~ ticks_per_trap + deer_other + (1 | inst_name/plot_id), 
  data = sem_data, family = binomial,
  glmerControl(optimizer = "Nelder_Mead", optCtrl=list(maxfun=1e6)))
# summary(human_mod_noRabbit)

human_mod_deer <- glmer(
  cbind(ticks_human, tick_abundance) ~ ticks_per_trap + deer + (1 | inst_name/plot_id), 
  data = sem_data, family = binomial,
  glmerControl(optimizer = "Nelder_Mead", optCtrl=list(maxfun=1e6)))
# summary(human_mod_deer)

human_mod_noHosts <- glmer(
  cbind(ticks_human, tick_abundance) ~ ticks_per_trap + (1 | inst_name/plot_id), 
  data = sem_data, family = binomial,
  glmerControl(optimizer = "Nelder_Mead", optCtrl=list(maxfun=1e6)))
# summary(human_mod_noHosts)

b <- AIC(human_mod_allHosts, human_mod_noRabbit, human_mod_deer, human_mod_noHosts)# essentially same information
