...
cross1_all_time_boxplot <- ggplot(cross1_data, aes(treatment, ring_width_mm, color = treatment)) +
  geom_boxplot(outlier.alpha = 1) +
  geom_point(alpha = 0.2, position= position_jitterdodge()) +
  #stripchart(ring_width_mm~year, data = cross1_data, vertical = T, method = "jitter") +
  #scale_x_continuous(labels = scaleFUN) +
  #scale_x_continuous(limits = c(2012, 2019), breaks = c(2013, 2014, 2015, 2016, 2017, 2018)) +
  #stat_summary(fun.data = "mean_se", geom = "pointrange", 
               #position = position_dodge(width = 0.7)) +
...
ggsave(plot = cross1_all_time_boxplot, height = 5, width = 10, "tree_rings/figures/width_crossSection1_boxplot.png")
...
# bivens_trees <- bivens_trees %>% 
#   filter(dbh_cm, !is.na(dbh_cm)) %>% 
#   filter(diam1_mm, !is.na(diam1_mm)) %>% 
#   filter(treatment, !is.na(treatment))

summary(bivens_trees)

tree_dbh_trt <- ggplot(bivens_trees, aes(treatment, dbh_cm, color = treatment)) +
  ...
  NULL
n_ch
tree_diam_trt <- ggplot(bivens_trees, aes(treatment, diam2_mm, color = treatment)) +
  geom_point(alpha = 0.25, position = "jitter") + 
  #scale_x_continuous(labels = scaleFUN) +
  #scale_x_continuous(limits = c(2012, 2019), breaks = c(2013, 2014, 2015, 2016, 2017, 2018)) +
  scale_x_discrete(breaks = c("AC", "AN", "DC", "DN")) +
  stat_summary(fun.data = "mean_se", geom = "pointrange", 
               position = position_dodge(width = 0.7)) +
  treatment_color +
  def_theme +
...
ggsave(plot = tree_diam_trt, height = 5, width = 10, "tree_rings/figures/tree_diam_trt.png")
...
soil_summary_plot <- soil_summary_plot %>% 
  filter(between(year,2014,2017)) 

soil_all_time <- ggplot(soil_summary_plot, aes(Treatment, average_vwc, color = Treatment)) +
  geom_point(alpha = 0.3, size = 1.85) + 
  #scale_x_continuous(labels = scaleFUN) +
  #scale_x_continuous(limits = c(2013, 2018), breaks = c(2014, 2015, 2016, 2017)) +
  stat_summary(fun.data = "mean_se", geom = "pointrange", 
               position = position_dodge(width = 0.7)) +
  treatment_color +
...
soil_all_time_boxplot <- ggplot(soil_summary_plot, aes(Treatment, average_vwc, color = Treatment)) +
  geom_boxplot(outlier.alpha = 1) +
...
ggsave(plot = soil_all_time_boxplot, height = 5, width = 10, "tree_rings/figures/soil_all_time_boxplot.png")
...
density_soil_moisture <- density_soil_moisture %>% 
  group_by(Plot, Treatment) %>% 
  summarise(average_vwc = mean(average_vwc),
...
ggsave(plot= tree_density_figure_two, width = 10, height = 6, "tree_rings/figures/tree_density_figure_two.png")

rw_diameter_trt_facet <- rw_diam_soil_density %>% 
  filter(between(year,2014,2017)) %>% 
  mutate(f_year = as.factor(year)) %>% 
  ggplot(aes(diameter, ring_width_mm)) +
  geom_point(aes(color = f_year), alpha = .44, size = 2) +
...
ggsave(plot = rw_soil_facet_trt, width = 15, height = 7, "tree_rings/figures/rw_soil_facet_trt.png")

rw_soil_facet_breakdown <- rw_soil_trees %>% 
  filter(between(year,2014,2017)) %>% 
  mutate(f_year = as.factor(year)) %>% 
  ggplot(aes(average_vwc, ring_width_mm)) +
  geom_point(aes(color = year), alpha = .7, size = 2) +
...
ggsave(plot = rw_soil_facet_breakdown, width = 15, height = 8, "tree_rings/figures/rw_soil_facet_breakdown.png")
