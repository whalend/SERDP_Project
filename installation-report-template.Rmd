...
output:
  word_document:
    toc: yes
    pandoc_args: [
    "--filter", "word-pagebreak-filter.R"] 
  html_document:
    self_contained: no
    toc: yes
    toc_float: yes
...
<<<<<<< HEAD
A total of `r n_total_raw_ticks_all` ticks were collected across all installations. At `r paste(inst_name)` in  ticks were collected and in `r n_raw_ticks_inst_2018` ticks were collected. Note the different tick abundance scales on each panel.
=======
A total of `r n_total_raw_ticks_all` ticks were collected across all installations. At `r paste(inst_name)` in `r n_raw_ticks_inst_2017` ticks were collected and in `r n_raw_ticks_inst_2018` ticks were collected. Note the different scales of the y-axis on each panel.
>>>>>>> 7df9713695d2e8e4a70f5be2fd6921e3dc7ec419
...
**Figure 5:** Across all installations, we see a general trend that more time since fire has a positive relationship with tick abundance in both native and invaded plots. Plot data points recorded specifically at `r inst_name` are recorded in `r installation_color`. Note there may be fewer points plotted than the total number of sample plots visited due to overlap.
...
**Figure 6:** A positive relationship is seen with percent litter cover and tick abundance across all installations. Litter cover is estimated by averaging of the litter cover in four 1m x 1m quadrats in each sampled plot. It is hypothesized that more litter cover creates a more suitable environment for tick survival (see Supplementary Information - Tick survival in cogongrass invasions). Plot data points recorded specifically at `r inst_name` are in `r installation_color`. Note there may be fewer points plotted than the total number of sample plots visited due to overlap.
...
**Figure 7:** A positive relationship is seen between litter biomass and tick abundance across all installations. Litter biomass estimates are from oven dried biomass collected at four 25cm x 25cm quadrats in each plot. Plot data points recorded specifically at `r inst_name` are in `r installation_color`. Note there may be fewer points plotted than the total number of sample plots visited due to overlap.
...
**Figure 9:** There was a positive relationship between tick abundance and overstory canopy cover in uninvaded plots. It is hypothesized denser canopy cover creates a more suitable microclimate at ground level for tick survival (see Supplementary Information - Tick survival in cogongrass invasions). Data recorded at `r inst_name` are shown in `r installation_color`. Note there may be fewer points plotted than the total number of sample plots visited due to overlap.
...
**Figure 11:** Overstory tree canopy cover tended to be greater with more time since fire. Data points at `r inst_name` are in `r installation_color`. Note there may be fewer points plotted than the total number of sample plots visited due to overlap.
...
**Figure 12:** There is a slight negative relationship between the number of host dung clusters and time since last fire in the plots with no cogongrass invasion; in invaded plots there is no trend. Data points at `r inst_name` are in `r installation_color`. Note there may be fewer points plotted than the total number of sample plots visited due to overlap.
...
**Figure 13:** There is a slight negative relationship between observed host dung clusters and overstory tree canopy cover across all plots. Data points at `r inst_name` are in `r installation_color`. Note there may be fewer points plotted than the total number of sample plots visited due to overlap.
...
**Figure 14:** We did not detect a relationship between time since fire and host abundance when looking at deer dung clusters only. Data points at `r inst_name` are recorded in `r installation_color`. Note there may be fewer points plotted than the total number of sample plots visited due to overlap.
...
**Figure 15:** We observed a positive relationship between litter cover and time since fire in both uninvaded and invaded plots and cogongrass-invaded plots. Uninvaded plots generally had greater litter cover. Data points at `r inst_name` are recorded in `r installation_color`. Note there may be fewer points plotted than the total number of sample plots visited due to overlap.
...
**Figure 16:**  There is a positive relationship litter biomass and time since fire in both uninvaded and cogongrass-invaded plots. Data points at `r inst_name` are recorded in `r installation_color`. Note there may be fewer points plotted than the total number of sample plots visited due to overlap.
...
**Figure 17:**  In uninvaded sites, there was a negative relationship with between herbaceous cover and time since fire. Increasing density of cogongrass invasions drastically increases vegetation cover as time since fire increases in cogongrass-invaded plots. Note that the sample size of invaded plots is very small. Data points at `r inst_name` are recorded in `r installation_color`. Note there may be fewer points plotted than the total number of sample plots visited due to overlap.
...
**Figure 18:** Across all plots there was a slight negative relationship between herbaceous species richness and time since fire. Species richness has large variability that is influenced by management and natural disturbances other than prescribed fire. Data points at `r inst_name` are recorded in `r installation_color`. Note there may be fewer points plotted than the total number of sample plots visited due to overlap.
...
**Figure 19:** There was a positive relationship between overstory species richness and time since fire. Only tree species with a DBH ≥ 3 cm and height ≥ 1m were recorded. Data points at `r inst_name` are recorded in `r installation_color`. Note there may be fewer points plotted than the total number of sample plots visited due to overlap.
...
**Figure 20:** No relationship in native sites with total tree DBH and time since fire. Total DBH is the cumulative value of all tree diameters recorded within the plot. Data points at `r inst_name` are recorded in `r installation_color`. Note there may be fewer points plotted than the total number of sample plots visited due to overlap.
...
Comparison of adult and nymph life stages of the lone star tick in native and cogongrass-invaded areas plots. All life stages experience greater mortality in native plots, with nymphs dying sonner than adults.

![](figures/tick-survival-assay/survivalfigsTogether.png)

**Figure:** Average tick survival (+ SE) over time at 12 locations in a cogongrass-invasion (red) and 12 locations in a nearby native understory (blue).
...
## Vegetation - quadrat measurements, biomass, and canopy cover 
...
kable(quadrat_species_fields, padding = 4, longtable = T, formate = "markdown") #%>%
  # kable_styling(bootstrap_options = "striped", full_width = F, position = "left") %>%
  # column_spec(1, bold = T, border_right = T) %>%
  # column_spec(2, border_right = T)
...
Some plot_id may contain NAs for all values in the sampled plot. These are placeholder zeros to show that the plot was sampled and no ticks were collected.
...
![](stem_maps/jackson a1.png)
\newpage

![](stem_maps/jackson b1.png)
\newpage
![](stem_maps/jackson c1.png)
\newpage
![](stem_maps/jackson d1.png)

\newpage

![](stem_maps/jackson e1.png)

\newpage

![](stem_maps/jackson f1.png)

\newpage

![](stem_maps/jackson g1.png)

\newpage

![](stem_maps/jackson h1.png)

\newpage

![](stem_maps/jackson i1.png)

\newpage

![](stem_maps/jackson j1.png)

\newpage

![](stem_maps/jackson k1.png)

\newpage

![](stem_maps/jackson l1.png)

\newpage

![](stem_maps/jackson m1.png)
>>>>>>> 7df9713695d2e8e4a70f5be2fd6921e3dc7ec419
