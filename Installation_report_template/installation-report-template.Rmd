...
params:
  installation_name: Fort Gordon
---

```{r set installation name options, echo=F}
# installation_name values to go in the params argument in the YAML header above
...
```{r load plot visit data processing}
# ASK ABOUT WORDING OF PLOT VISIT VS TOTAL
...
plot_visit_data <- plot_visit_data %>% 
  ...
  rename(installation_full_name=inst_name)
...
n_plots_invaded_inst <- n_distinct(filter(plot_visit_data, imcy_inv=="Invaded" & installation_full_name==inst_name))
...
tick_data_life_stage_summary <- tick_data %>% 
      group_by(plot_id, visit_year, life_stage, years_since_fire, imcy_inv, installation_full_name, visit_date) %>% 
