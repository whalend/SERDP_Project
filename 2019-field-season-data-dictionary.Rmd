---
title: "2019 SERDP Field Season Data Dictionary"
...
other 2019 only datasheets to add for data dictionary:

camera_traps_info = description of camera numbers and plot location

all_camera_photos_raw = raw photos ~18000
camera_trap_photos_all = trimmed processed photos, no hancock n1 or avon park, 17300 photos

camera_trap_days = each plot with just time in and out, number of days per plot with each camera based on min max timepoints calculated from photo timestamp

camera-species-with-days-by-sd-card = species summarized down to the species sum per sd card

species-per-plot-per-day = species sumarized to plot level, if there are more than one camera per plot, those cameras are summed in number of days recorded and number of wildlife photos, species totals

species-per-camera-per-day = similar to camera-species-with-days-by-sd-card, but added divided by per day factor to get number of wildlife seen per day
