...
struc_colors <- scale_color_manual(values=c("red","cyan"))
struc_fill <- scale_fill_manual(values=c("red","cyan"))
...
shapes <- scale_shape_manual(values = c("prescribed" = 23, "piled" = 2, "standing" = 0, "both" = 19))

colors <- scale_color_manual(values = c("piled" = "red", "standing" = "deepskyblue", "prescribed" = "black", "both" = "purple"))
...
ggplot(d1, aes(structure, pct_fuel_moisture)) +
      ...
      colors + 
      scale_fill_manual(values = c("both"="purple","piled"="red","standing"="deepskyblue")) +
      shapes +
      linetypes +
...
max_temp_fig <- ggplot(d1, aes(total_biomass, avg2_max_temp)) +
      ...
      geom_errorbar(data = p_fire_summary, aes(x = avg_biomass, y = avg_max_temp, ymax = avg_max_temp + sd_max_temp, ymin = avg_max_temp - sd_max_temp), width = .1) +
      geom_errorbarh(data = p_fire_summary, aes(x = avg_biomass, y = avg_max_temp, xmin = avg_biomass - sd_biomass, xmax = avg_biomass + sd_biomass), height = .1) +
      geom_point(data = p_fire_summary, aes(avg_biomass, avg_max_temp, shape = structure, color = structure), size = 2, fill = "gold") +
      # geom_boxplot(data = prescribed_fire, aes(total_biomass, max_temp)) +
      ## ##
            
      scale_shape_manual(values = c("piled" = 2, "standing" = 0, "both" = 19, "prescribed" = 23), breaks = legend_order) +
      scale_color_manual(values = c("piled" = "red", "standing" = "deepskyblue", "both" = "purple", "prescribed" = "black"), breaks = legend_order) +
      scale_fill_manual(values = c("piled"="red","standing"="cyan", "both"="purple"), breaks = legend_order) +
