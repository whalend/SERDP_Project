...
library(knitr); 
library(plyr); library(dplyr); library(readr); library(ggplot2); library(tidyr)
...
## Fire Ecology figure guidelines ####
...
legend_order <- c("piled","standing","both", "prescribed")

fig_shapes <- scale_shape_manual(values = c("prescribed" = 23, "piled" = 2, "standing" = 0, "both" = 19), breaks = legend_order)

fig_colors <- scale_color_manual(values = c("piled" = "red", "standing" = "deepskyblue", "prescribed" = "black", "both" = "purple"), , breaks = legend_order)

fig_fills <- scale_fill_manual(values = c("both" = "purple", "piled" = "red", "standing" = "deepskyblue"), breaks = legend_order)

fig_linetypes <- scale_linetype_manual(values = c("piled" = "solid", "standing" = "solid", "both" = "dashed"), breaks = legend_order)
...
A better understanding of how fuel characteristics affect fire intensity could improve predictions about fire behavior and severity. Current methods for evaluating the effects of fuels on fire intensity range from small-scale laboratory studies that can be precise and highly replicated but may not scale-up to represent realistic fires, to manipulative field experiments, which capture conditions during real-world fires but can suffer from imprecise fuel and fire intensity measurements and low replication. We present an innovative, low-cost method to experimentally
manipulate fuels and measure fire intensity that bridges the gap between laboratory techniques and fire conditions in the field. Using an invasive grass that is widespread in the Southeast US, we demonstrate that maintaining vertical fuel structure using our method significantly affects fire intensity metrics across five different fuel loads compared to piled fuels. Comparisons with measurements from a prescribed fire indicate that this method is an effective surrogate for fires in the field. This method can be used to experimentally determine the effects of manipulating fuel structure and load on fire intensity for a wide range of systems and fuels, thereby improving predictions of how fuel characteristics affect fire severity
...
We used ordinary least squares linear regressions to model the response variables of average maximum temperature, time above 100 ºC at each probe height, the average flame height, and the rate of spread. We used beta regression to model the proportion of biomass consumed. Fuel load (mass) and fuel structure (piled vs. standing) were used as explanatory variables in each model, and fuel load was entered as a continuous variable. We tested the main effects and interaction of fuel load and fuel structure along with weather covariates (e.g. air temperature, wind speed, relative humidity). Statistically insignificant weather variables were excluded from final models. Because our main goal was to assess the effect of fuel structure (piled vs. standing) on measures of fire intensity and behavior we at minimum retained fuel structure and biomass as main effects in all models.
...
ggplot(d1, aes(total_biomass, s_abv100)) +
      ...
      scale_color_manual(values = c("both"="purple","piled"="red","standing"="deepskyblue")) +
...
# names(fires_biomass)
d1$structure <- factor(d1$structure, levels = c("piled","standing","both"))
# all_fabio_fires_biomass
...
#       mutate(se_moisture = sd_moisture/sqrt(nobs)) %>% 
#       knitr::kable(., digits = 2)

t.test(filter(d1, structure=="piled")$pct_fuel_moisture, filter(d1, structure=="standing")$pct_fuel_moisture)
shapiro.test(filter(d1, structure=="piled")$pct_fuel_moisture)
shapiro.test(filter(d1, structure=="standing")$pct_fuel_moisture)
car::leveneTest(pct_fuel_moisture~structure, data = filter(d1, structure!="both"), center = "mean")
car::leveneTest(pct_fuel_moisture~structure, data = filter(d1, structure!="both"), center = "median")

fm <- lm(pct_fuel_moisture ~ structure, data = d1)
anova(fm)
aov(fm)
TukeyHSD(aov(fm))


t.test(filter(d1, structure=="piled")$pct_fuel_moisture, filter(d1, structure=="both")$pct_fuel_moisture)

t.test(filter(d1, structure=="standing")$pct_fuel_moisture, filter(d1, structure=="both")$pct_fuel_moisture)
      
```

```{r field sampled cogon moisture}
serdp_cogon <- read_csv("data/processed_data/cogongrass.csv")
cogon_moisture <- serdp_cogon %>% 
  select(date, fresh_biomass, dry_biomass) %>% 
  filter(!is.na(dry_biomass)) %>% 
  mutate(pct_moisture = 100*(fresh_biomass - dry_biomass)/fresh_biomass,
         week = lubridate::week(date))

fm <- cogon_moisture$pct_moisture

fm <- c(fm, 13.9, 17, 13.4, 16.1, 17.9, 13.4)
summary(fm)
sd(fm)
length(fm)

ggplot(cogon_moisture, aes(week, pct_moisture, group = week)) +
  geom_point(position = "jitter")
summary(cogon_moisture)
```
...
knitr::kable(select(p_fire_summary, location, avg_max_temp:se_s_abv100), digits = 2)
...
d1 %>% 
      filter(avg2_max_temp > 750) %>% 
      arrange(avg2_max_temp)
...
max_temp_lm_full <- glm(avg2_max_temp ~ total_biomass + total_biomass*structure + total_biomass*location + structure:location + pct_fuel_moisture,
...
# 0cm location ####
### pct_fuel_moisture not significant
...
## pct_fuel_moisture not significant
max_temp_50cm <- lm(avg2_max_temp ~ total_biomass + structure + avg_air_temp, data = fabio_fires_50cm)
summ_max_temp_50cm <- summary(max_temp_50cm)
# summ_max_temp_50cm$adj.r.squared
sjPlot::plot_model(max_temp_50cm, type = "pred", terms = c("total_biomass","structure","avg_air_temp"))

my_row_names <- c("Intercept", "Biomass", "Structure: standing", "Air temperature")
...
maxtemp_fig_labels <- data.frame(
      location = factor(c("50cm","25cm","0cm")),
      pid = c("d","e","f"), 
      total_biomass = c(200,200,200),
      avg2_max_temp = c(rep(840, 3))
      )


maxtemp0cm_modelplot <- sjPlot::plot_model(max_temp_0cm, type = "pred", terms = c("total_biomass","structure"))
maxtemp0cm_modelplot$data$location="0cm"
maxtemp0cm_modelplot$data$location=factor(maxtemp0cm_modelplot$data$location, levels = c("50cm","25cm","0cm"))

maxtemp25cm_modelplot <- sjPlot::plot_model(max_temp_25cm, type = "pred", terms = c("total_biomass","structure"))
maxtemp25cm_modelplot$data$location="25cm"
maxtemp25cm_modelplot$data$location=factor(maxtemp25cm_modelplot$data$location, levels = c("50cm","25cm","0cm"))

maxtemp50cm_modelplot <- sjPlot::plot_model(max_temp_50cm, type = "pred", terms = c("total_biomass","structure"))
maxtemp50cm_modelplot$data$location="50cm"
maxtemp50cm_modelplot$data$location=factor(maxtemp50cm_modelplot$data$location, levels = c("50cm","25cm","0cm"))

fabio_fires_0cm$location = factor(fabio_fires_0cm$location, levels = c("50cm","25cm","0cm"))
fabio_fires_25cm$location = factor(fabio_fires_25cm$location, levels = c("50cm","25cm","0cm"))
fabio_fires_50cm$location = factor(fabio_fires_50cm$location, levels = c("50cm","25cm","0cm"))


## The prediction for 50cm is with air temperature at its mean of 26.55
max_temp_fig <- ggplot() +
      
      ## 50cm model
      
      geom_ribbon(data = maxtemp50cm_modelplot$data, aes(x = x, ymin = conf.low, ymax = conf.high, fill = group), alpha = .2, show.legend = F) +
      geom_smooth(data = maxtemp50cm_modelplot$data, aes(x = x, y = predicted, color = group), method = "lm", show.legend = F) +
      
      geom_point(data = rename(fabio_fires_50cm, group=structure), aes(x = total_biomass, y = avg2_max_temp, color = group, shape = group)) +
      
      # geom_smooth(data = filter(d1, structure=="both", location=="50cm") %>% 
      #                   rename(group = structure),
      #             aes(x = total_biomass, y = avg2_max_temp, linetype = group, color = group), method = "lm", se = F, show.legend = F) +
      
      # geom_point(data = filter(d1, structure=="both", location=="50cm") %>% 
      #                   rename(group = structure),
      #             aes(x = total_biomass, y = avg2_max_temp, shape = group, color = group), show.legend = F) +
      
      ## 25cm model
      
      geom_ribbon(data = maxtemp25cm_modelplot$data, aes(x = x, ymin = conf.low, ymax = conf.high, fill = group), alpha = .2, show.legend = F) +
      geom_smooth(data = maxtemp25cm_modelplot$data, aes(x = x, y = predicted, color = group), method = "lm", show.legend = F) +
      
      geom_point(data = rename(fabio_fires_25cm, group=structure), aes(x = total_biomass, y = avg2_max_temp, color = group, shape = group)) +
      
      # geom_smooth(data = filter(d1, structure=="both", location=="25cm") %>% 
      #                   rename(group = structure),
      #             aes(x = total_biomass, y = avg2_max_temp, linetype = group, color = group), method = "lm", se = F, show.legend = F) +
      # 
      # geom_point(data = filter(d1, structure=="both", location=="25cm") %>% 
      #                   rename(group = structure),
      #             aes(x = total_biomass, y = avg2_max_temp, shape = group, color = group), show.legend = F) +
      
      ## 0cm model
      
      geom_ribbon(data = maxtemp0cm_modelplot$data, aes(x = x, ymin = conf.low, ymax = conf.high, fill = group), alpha = .2, show.legend = F) +
      geom_smooth(data = maxtemp0cm_modelplot$data, aes(x = x, y = predicted, color = group), method = "lm", show.legend = F) +
      
      geom_point(data = rename(fabio_fires_0cm, group=structure), aes(x = total_biomass, y = avg2_max_temp, color = group, shape = group)) +
      
      
      # geom_smooth(data = filter(d1, structure=="both", location=="0cm") %>% 
      #                   rename(group = structure),
      #             aes(x = total_biomass, y = avg2_max_temp, linetype = group, color = group), method = "lm", se = F, show.legend = F) +
      # 
      # geom_point(data = filter(d1, structure=="both", location=="0cm") %>% 
      #                   rename(group = structure),
      #             aes(x = total_biomass, y = avg2_max_temp, shape = group, color = group), show.legend = F) +
      ...
      # 
      # geom_point(data = rename(p_fire_summary, group=type), aes(avg_biomass, avg_max_temp, shape = group, color = group), size = 2, fill = "gold") +
      
      geom_text(data = maxtemp_fig_labels, 
                aes(x=total_biomass, y=avg2_max_temp, label = pid),
                fontface = "bold") +
      
      fig_fills + fig_colors + fig_shapes + fig_linetypes +
...
s_abv100_lm <- lm(s_abv100 ~ total_biomass + pct_fuel_moisture + total_biomass*structure + total_biomass*location + structure*location, data = d2, na.action = na.fail)
# summary(s_abv100_lm)
...
sjp_sabv_0cm <- sjPlot::plot_model(s_abv100_0cm, type = "pred", terms = c("total_biomass","structure"))
sjp_sabv_0cm$data$location="0cm"
sjp_sabv_0cm$data$location=factor(sjp_sabv_0cm$data$location, levels = c("50cm","25cm","0cm"))

sjp_sabv_25cm <- sjPlot::plot_model(s_abv100_25cm, type = "pred", terms = c("total_biomass","structure"))
sjp_sabv_25cm$data$location="25cm"
sjp_sabv_25cm$data$location=factor(sjp_sabv_25cm$data$location, levels = c("50cm","25cm","0cm"))

sjp_sabv_50cm <- sjPlot::plot_model(s_abv100_50cm, type = "pred", terms = c("total_biomass","structure"))
sjp_sabv_50cm$data$location="50cm"
sjp_sabv_50cm$data$location=factor(maxtemp50cm_modelplot$data$location, levels = c("50cm","25cm","0cm"))

sabv_fig_labels <- data.frame(location = factor(c("50cm","25cm","0cm")), pid = c("a","b","c"), total_biomass = c(200,200,200), s_abv100 = c(160,200,775))
...
s_abv100_fig <- ggplot() +
      ...
      geom_ribbon(data = sjp_sabv_50cm$data, aes(x = x, ymin = conf.low, ymax = conf.high, fill = group), alpha = .2, show.legend = F) +
      
      geom_smooth(data = sjp_sabv_50cm$data, aes(x = x, y = predicted, color = group), method = "lm", show.legend = F) +
      
      geom_point(data = rename(fabio_fires_50cm, group=structure), aes(x = total_biomass, y = s_abv100, color = group, shape = group)) +
      
      # geom_smooth(data = filter(d1, structure=="both", location=="50cm") %>% 
      #                   rename(group = structure),
      #             aes(x = total_biomass, y = s_abv100, linetype = group, color = group), method = "lm", se = F, show.legend = F) +
      # 
      # geom_point(data = filter(d1, structure=="both", location=="50cm") %>% 
      #                   rename(group = structure),
      #             aes(x = total_biomass, y = s_abv100, shape = group, color = group), show.legend = F) +
      
      ## 25cm model
      
      geom_ribbon(data = sjp_sabv_25cm$data, aes(x = x, ymin = conf.low, ymax = conf.high, fill = group), alpha = .2, show.legend = F) +
      
      geom_smooth(data = sjp_sabv_25cm$data, aes(x = x, y = predicted, color = group), method = "lm", show.legend = F) +
      
      geom_point(data = rename(fabio_fires_25cm, group=structure), aes(x = total_biomass, y = s_abv100, color = group, shape = group)) +
      ...
      geom_ribbon(data = sjp_sabv_0cm$data, aes(x = x, ymin = conf.low, ymax = conf.high, fill = group), alpha = .2, show.legend = F) +
      
      geom_smooth(data = sjp_sabv_0cm$data, aes(x = x, y = predicted, color = group), method = "lm", show.legend = F) +
      
      geom_point(data = rename(fabio_fires_0cm, group=structure), aes(x = total_biomass, y = s_abv100, color = group, shape = group)) +
      ...
      # 
      # geom_point(data = rename(p_fire_summary, group=type), aes(avg_biomass, avg_s_abv100, shape = group, color = group), size = 2, fill = "gold") +
      
      geom_text(data = sabv_fig_labels, 
                aes(x=total_biomass, y=s_abv100, label = pid),
                fontface = "bold") +
...
## Multi-panel temperature figure for publication
...
# ggsave("FABIO-manuscript/figures/maxt_models_fig.png", tfig, width = 3.25, height = 7.5, dpi = 600, device = "png")

# ggsave("FABIO-manuscript/figures/sabv_models_fig.png", sfig, width = 3.25, height = 7.5, dpi = 600, device = "png")
...
cowplot::plot_grid(
  ...
  cowplot::draw_label(
    expression(paste("Fuel load (g ", m^{-2}, ")")), 
    x = 0.5, y = 0, vjust=-0.2, angle= 0) 
...
```{r fuel height figure}

ggplot(fires_biomass, aes(structure, avg_fuel_ht)) +
      geom_boxplot()
...
```{r flame height table}
fires_biomass %>% 
  group_by(structure) %>% 
  summarise(avg_max_flame_ht = mean(max_flame_ht),
            avg_fht = mean(avg_flame_ht),
            max_flame_ht = max(max_flame_ht)
            )
```
...
f_ht_mod <- lm(avg_flame_ht ~ biomass*structure + avg_air_temp , data = fires_biomass)
# summary(f_ht_mod)
# sjPlot::plot_model(f_ht_mod, type = "int")
# no RH, wind speed, fuel height

f_ht_mod2 <- lm(avg_flame_ht ~ biomass*structure + avg_fuel_ht, data = fires_biomass)
# summary(f_ht_mod2)
## Measured fuel height doesn't have an effect per se beyond the different structure, which I think makes sense because are fuel heights within a structure didn't have a wide range.
# sjPlot::plot_model(f_ht_mod2, type = "pred", terms = c("biomass", "structure"))
## Test for effect of fuel height within the structure treatment
f_ht_mod2a_piled_only <- lm(avg_flame_ht ~ biomass + max_fuel_ht, data = filter(fires_biomass, structure=="piled"))
summary(f_ht_mod2a_piled_only)

f_ht_mod2a_standing_only <- lm(avg_flame_ht ~ biomass + max_fuel_ht, data = filter(fires_biomass, structure=="standing"))
summary(f_ht_mod2a_standing_only)
## Avg and max fuel height is significant for piled but not for standing
...
flame_ht_mod_summ <- summary(f_ht_mod)

my_row_names <- c("Intercept", "Biomass", "Structure: standing", "Air temperature", "Biomass X Standing")
...
# require(car)
f_ht_anova <- car::Anova(f_ht_mod, type = "II")
...
p1 <- sjPlot::plot_model(f_ht_mod, type = "pred", terms = c("biomass","structure"), show.data = F, show.legend = FALSE)

library(ggeffects)

df <- ggpredict(f_ht_mod, terms = c("biomass","structure"))

flame_height_fig <- ggplot(df, aes(x, predicted)) +
      
      geom_ribbon(aes(ymin = conf.low, ymax = conf.high, color = group, fill = group), alpha = .2, show.legend = FALSE, linetype = 0) +
      
      geom_point(data = filter(d1, location =="0cm", structure!="both") %>% 
                       rename(group = structure), 
                 aes(total_biomass, avg_flame_ht, 
                     color = group, shape = group)) +
      
      # geom_smooth(
      #       data = filter(d1, structure == "both", location=="0cm") %>% 
      #             rename(group = structure),
      #       aes(total_biomass, avg_flame_ht, 
      ...
      geom_smooth(aes(color = group), method = "lm", show.legend = F, se = FALSE) +
...
pct_consumed_data <- d1 %>% filter(!duplicated(fire_id)) %>% 
      select(fire_id, structure, total_biomass, pct_consumed) %>% 
      mutate(p_consumed = pct_consumed*.01,
             p_consumed = if_else(p_consumed==1, .99, p_consumed),
             pct_consumed = p_consumed * 100) %>% 
      left_join(., select(fires_biomass, fire_id, pct_fuel_moisture, avg_air_temp:avg_wind_speed))
...
pct_consumed_fig <- ggplot(pct_consumed_data, aes(total_biomass, pct_consumed)) +
      ...
      geom_smooth(data = filter(pct_consumed_data, structure=="piled"), aes(color = structure, fill = structure, linetype = structure), method = "lm", se = F, size = .8, alpha = .2, show.legend = F) +
      
      geom_point(data = filter(pct_consumed_data, structure!="both"),aes(color = structure, shape = structure)) +
      
      scale_shape_manual(values = c("prescribed" = 23, "piled" = 2, "standing" = 0, "both" = 19), breaks = legend_order) +
      scale_color_manual(values = c("piled" = "red", "standing" = "deepskyblue", "prescribed" = "black", "both" = "purple"), breaks = legend_order) +
      scale_fill_manual(values = c("both" = "purple", "piled" = "red", "standing" = "cyan"), breaks = legend_order) +
      scale_linetype_manual(values=c("piled" = "solid", "standing" = "solid", "both" = "dashed"), breaks = legend_order) +
...
# library(betareg)
pct_consumed_breg <- betareg::betareg(p_consumed ~ total_biomass*structure + avg_rh, data = pcons_model_data)
...
MuMIn::AICc(m2, m3, pct_consumed_breg)
...
sjp.p.consumed <- sjPlot::plot_model(pct_consumed_breg, type = "pred", terms = c("total_biomass","structure"), show.data = F)

pct_consumed_fig <- sjp.p.consumed + 
      geom_ribbon(
        data = sjp.p.consumed$data, 
        aes(x = x, ymin = conf.low, ymax = conf.high, fill = group_col),
        alpha = .2, linetype = 0, show.legend = FALSE) +

      geom_point(
        data = filter(pct_consumed_data, structure!="both") %>%
          rename(group_col = structure),
        aes(total_biomass, p_consumed, color = group_col, 
            shape = group_col), 
        show.legend = F) +
...
fires_biomass %>% 
      group_by(structure) %>% 
      summarise(mean(rate_of_spread_ms))
...
sjp.ros <- sjPlot::plot_model(ros_model1, type = "pred", terms = c("total_biomass","structure"), show.data = FALSE)

ros_fig <- sjp.ros +
      
      geom_point(data = rename(fires_biomass, group_col = structure),
            aes(x = total_biomass, y = rate_of_spread_ms, color = group_col, shape = group_col)) +
      ...
      fig_colors + fig_shapes + fig_fills + fig_linetypes +
...
## Multi-panel publication figure: flame height, ros, % consumed
...
cowplot::plot_grid(
      flame_height_fig +
            ...
            theme(legend.background = element_blank(),
                  legend.key = element_blank(),
                  legend.position = c(-.01, 1.1),
                  # legend.spacing.y = unit(.5, units = "cm"),
                  plot.margin = margin(c(15,5,-5,15))),
      ros_fig + 
            xlab("") +
            theme(legend.position = "none",
                  plot.margin = margin(c(-5,5,-5,15))),
      pct_consumed_fig +
            theme(legend.position = "none",
                  plot.margin = margin(c(-5,5,0,15))),
      ncol = 1, labels = "auto", align = "v")

ggsave("figures/ros-flame-consumed.pdf", height = 17, width = 8.5, units = "cm", dpi = 600)
...
```{r write out model dataframes, eval=F}

t_mod_data <- d2 %>% 
      select(fire_id, location, structure, total_biomass,
             avg_max_temp = avg2_max_temp,
             s_abv100,
             pct_fuel_moisture, avg_air_temp:avg_wind_speed)
write_csv(t_mod_data, "FABIO-manuscript/temperature_data.csv")

flameht_ros_data <- fires_biomass %>% 
      select(fire_id, structure, biomass, rate_of_spread_ms, avg_flame_ht,
             pct_fuel_moisture, avg_air_temp:avg_wind_speed)
write_csv(flameht_ros_data, "FABIO-manuscript/flameht_ros_data.csv")

write_csv(pct_consumed_data %>% 
      filter(structure!="both") %>% 
      select(fire_id:total_biomass, p_consumed,
       pct_fuel_moisture:avg_wind_speed),
      "FABIO-manuscript/pct_consumed_data.csv"
)
...
moisture <- d1 %>% 
  select(fire_id, pct_fuel_moisture) %>% 
  distinct(fire_id, pct_fuel_moisture)

fire_wx_table <- left_join(
  fire_wx_table, moisture
)

fire_wx_table <- fire_wx_table %>% mutate(fire_id = seq_len(length(fire_id)))

wxtable_col_names <- c("Fire ID", "Date", "Structure", "Biomass", "Air temperature (ºC)", "RH (%)", "Wind Speed (m s^-1^)", "Fuel moisture (%)")
