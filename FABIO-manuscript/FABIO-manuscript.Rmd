...
For all fires we used HOBO J,K,S,T thermocouple loggers (Model U12, Onset Computer Corporation, Bourne MA) with type-K thermoucouples resulting in a measurement range of 0 to 1250 ºC. In the experimental fires we used wire probes insulated in braided steel that were 91.4 cm long with a diamter of XX mm. These probes were robust to exposure to multiple fires and could be used continuously. During the prescribed fire the thermocouple wire was fiberglass insulated and any length of wire exposed to flames had to be removed. Although the braided steel insulated probes were more expensive they were ultimately more economical because they didn't degrade when exposed to flames.

Weather data was recorded at a two second interval during fires using a Kestrel 5500 Fire Weather Pro pocket weather tracker (Nielsen-Kellerman, Boothwyn, PA) mounted on a tripod 1 m above the ground. Data points were matched to the time of each fire and average values were calculated for air temperature, relative humidity, and wind speed.
...
# all_fabio_fires_biomass
ggplot(d1, aes(structure, est_pct_fuel_moisture)) +
...
d1 %>% 
      ...
      group_by(structure) %>% 
      summarise(nobs = n(),
                min_moisture = min(est_pct_fuel_moisture, na.rm = T),
                avg_moisture = mean(est_pct_fuel_moisture, na.rm = T),
                max_moisture = max(est_pct_fuel_moisture, na.rm = T),
                sd_moisture = sd(est_pct_fuel_moisture, na.rm = T),
                se_moisture = sd_moisture/sqrt(nobs)) %>% 
      knitr::kable(., digits = 2)
      
```

Across all experimental fires, fuel moisture content ranged from 4.6 to 19.6% (mean = 11.5 ± 0.6% SE). Moisture of fuels used for piled fires ranged from 4.6 to 15.3% (mean = 11.2 ± 0.7% SE) and for standing fires from 6.8 to 19.6% (mean = 11.8 ± 0.9% SE). Fuel moisture was 
...
```{r flame ros consumed fig, fig.height=7.5, fig.width=4}
