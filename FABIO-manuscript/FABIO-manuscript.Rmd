...
d2
(pct_consumed_fig <- ggplot(fires_biomass, aes(total_biomass, pct_consumed, color = structure)) +
      struc_colors +
      # geom_smooth(aes(color = structure, fill = structure, linetype = structure), method = "lm", formula = y~log(x), se = T, size = .8, alpha = .2) +
      stat_summary(fun.data = "mean_se", geom = "pointrange", position = position_dodge(width = 100), aes(shape = structure)) +
      geom_point(aes(shape = structure), alpha = .2, position = position_dodge(width = 100)) +
      # geom_point() +
      theme_bw() +
      xlab(expression(paste("Fuel load (g ", m^{-2}, ")"))) +
      ylab("Mass consumed (%)") +
...
```{r pct consumed model}

# summary(glm(pct_consumed ~ biomass*structure, data = fires_biomass, family ="inverse.gaussian"))
# car::Anova(pct_consumed ~ biomass*structure, data = fires_biomass)

pct_consumed_data <- fires_biomass %>% 
      select(biomass, structure, pct_consumed)
pct_consumed_breg <- betareg::betareg(pct_consumed*.01 ~ biomass*structure, data = fires_biomass)
