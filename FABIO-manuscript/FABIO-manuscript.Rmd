...
*Keywords*: fine fuels, fire heterogeneity, fire intensity, fuel load, fuel structure
...
Fine surface fuels play a major role in the ignition, spread, and intensity of fires. Fire behavior from ~1-m^2^ to landscape (hectares) scales is driven by weather conditions and fuel charateristics, including load, structure, moisture, and continuity. Fuel load has been shown to be a particularly important driver of combustibility, sustainability, and rate of spread, characteristics. 

Understanding how fuel characteristics influence fire intensity, severity, and behavior is imperative for fire ecologists, modelers, and land managers where fire is an integral part of the landscape. However, specific information is know about relatively few species, and while valuable, this information is often obtained by studies conducted across a spectrum of realistic conditions. Methods range from *in situ* approaches where measurements are made before, during, and after prescribed fires (high realism, low replication), to laboratory bench approaches where small amounts of fuel are combusted in highly controlled settings (low realism, highly replicable). 

Studies in fire ecology that experimentally manipulate fuel often only change the fuel load or type, necessarily sacrificing some realism in fuel structure. Fuels are often laid horizontally (e.g.  @Thaxton2006, @JAUREGUIBERRY2011, @Simpson2016, @Wyse2016, @Bowman2017) even though fuel complexes are often more heterogeneous with subtantial vertical structure. For example, @Bowman2017 laid sorghum grass fuels horizontally in a field experiment examining how fine fuel loads of different types affected fire intensity and survival of tree seedlings. 

In this paper, we present an experimental apparatus that can bridge the gap between making measurements of flammability at the lab bench versus prescribed fires. The Fine Aboveground Biomass Incineration Organizer (FABIO) enables experimental manipulation of fuel characteristics such as load and structure at a realtively small but realistic and relevant scale of 1-m^2^ (@Loudermilk2014). We expect our design will be most useful for grasses or grass-like fuels, but it could be used or adapted for other fuel types such as small shrubs and trees. 
...
Across all fires, fuel moisture content ranged from 4.6 to 19.6 % (11.49±0.56). 
...
max_temp_0cm <- lm(max_temp ~ biomass + structure -1, data = fabio_fires_0cm)
...
max_temp_25cm <- lm(max_temp ~ biomass, data = fabio_fires_25cm)

my_row_names <- c("(Intercept)", "Biomass")
...
### Linear models of time above 100 ºC
...
### Linear models of rate of spread
...
ggplot(fires_biomass, aes(biomass, rate_of_spread_ms, color = structure)) +
      theme_bw() +
      xlab(expression(paste("Fuel load (g/", m^{2}, ")"))) +
      ylab(expression(paste("Rate of spread (m", s^{-1}, ")"))) +
      geom_smooth(aes(linetype = structure), method = "lm") +
      geom_point(size = 3) +
      scale_color_discrete(h.start = 20) +
      theme(legend.title = element_blank(), 
            # legend.direction = "vertical",
