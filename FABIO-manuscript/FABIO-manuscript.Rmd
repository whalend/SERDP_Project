...
ggplot(fires_biomass, aes(structure, pct_fuel_moisture)) +
      geom_boxplot(aes(color = structure)) +
      stat_summary(aes(fill = structure), fun.y = "mean", geom = "point", shape = 22, size = 3, show.legend = F) +
      geom_point(aes(color = structure),size = 1, alpha = 1, show.legend = F) +
      struc_colors +
      struc_fill +
      theme_classic() +
...
fires_biomass %>% 
      group_by(structure) %>% 
      summarise(nobs = n(),
                min_moisture = min(pct_fuel_moisture),
                avg_moisture = mean(pct_fuel_moisture),
                max_moisture = max(pct_fuel_moisture),
                sd_moisture = sd(pct_fuel_moisture),
                se_moisture = sd_moisture/sqrt(nobs)) %>% 
      knitr::kable(., digits = 2)
      
```

Across all fires fuel moisture content ranged from 4.6 to 19.6% (mean = 11.5 ± 0.6% SE). Moisture of fuels used for piled fires ranged from 4.6 to 15.3% (mean = 11.2 ± 0.7% SE) and for standing fires from 6.8 to 19.6% (mean = 11.8 ± 0.9% SE). 
...
```{r flame ros consumed fig, fig.height=8, fig.width=4}
