...
library(MuMIn)
dd <- dredge(max_temp_lm_full, extra = c("adjR^2"))
# filter(dd, delta < 10)

my_row_names <- c("(Intercept)", "Biomass", "Structure: standing")
max_temp_0cm <- glm(max_temp ~ (biomass) + structure, data = filter(fabio_fires, location == "0cm"))
...
max_temp_25cm <- lm(max_temp ~ (biomass) + structure, data = filter(fabio_fires, location == "25cm"))
...
max_temp_50cm <- lm(max_temp ~ (biomass) + structure, data = filter(fabio_fires, location == "50cm"))
...
coef25cm <- as.data.frame(summary(s_abv100_0cm)$coefficients)
rownames(coef25cm) <- my_row_names
...
coef50cm <- as.data.frame(summary(s_abv100_0cm)$coefficients)
rownames(coef50cm) <- my_row_names
