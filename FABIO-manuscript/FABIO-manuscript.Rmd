...
all_fabio_fires_biomass <- fires_biomass1 %>% 
            ...
            select(fire_id, structure, total_biomass, pct_piled, rate_of_spread_ms, avg_flame_ht, litter_biomass, pct_fuel_moisture, est_pct_fuel_moisture)
...
(max_temp_fig <- ggplot(fabio_fires, aes(biomass, avg2_max_temp)) +
      # scale_color_discrete(h.start = 20) +
      ...
      geom_smooth(aes(color = structure, linetype = structure, fill = structure), method = "lm", se = T, size = .8, alpha = .2, show.legend = F) +
      ...
      geom_point(data = p_fire_summary, aes(avg_biomass, avg_max_temp, shape = type, color = type), size = 2.5) +
      ...
      scale_shape_manual(values = c("natural" = 15, "piled" = 16, "standing" = 17)) +
      scale_color_manual(values = c("piled" = "red", "standing" = "blue", "natural" = "gold")) +
...
max_temp_lm_full <- glm(max_temp ~ biomass + biomass*structure + biomass*location + structure:location + avg_air_temp + avg_rh, 
...
```{r temperature-combined-figure, fig.height=6}
...
ggsave("figures/temperature-fig.png", width = 7.5, height = 6)
...
f_ht_mod2 <- lm(avg_flame_ht ~ biomass + structure, data = fires_biomass)
summary(f_ht_mod2)
sjPlot::plot_model(f_ht_mod2, type = "pred", terms = c("biomass", "structure"))

AIC(f_ht_mod, f_ht_mod2)
anova(f_ht_mod, f_ht_mod2)
...
(pct_consumed_fig <- ggplot(fires_biomass, aes(biomass, pct_consumed, color = structure)) +
      struc_colors +
      geom_point(alpha = .2) +
      # geom_smooth(aes(color = structure, fill = structure, linetype = structure), method = "lm", formula = y~log(x), se = T, size = .8, alpha = .2) +
      stat_summary(fun.data = "mean_se", geom = "pointrange", position = position_dodge(width = 100), aes(shape = structure)) +
      # geom_point() +
      theme_bw() +
      xlab(expression(paste("Fuel load (g ", m^{-2}, ")"))) +
