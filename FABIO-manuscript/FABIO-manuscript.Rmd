...
sabv_fig_labels <- data.frame(location = c("50cm","25cm","0cm"), pid = c("a","b","c"), total_biomass = c(200,200,200), s_abv100 = c(160,200,775))
...
pct_consumed_fig <- sjp.p.consumed + 
      ...
      geom_ribbon(data = sjp.p.consumed$data, aes(x = x, ymin = conf.low, ymax = conf.high, fill = group), alpha = .2, linetype = 0, show.legend = FALSE) +

      geom_point(data = filter(pct_consumed_data, structure!="both") %>% rename(group = structure),
                 aes(total_biomass, p_consumed, 
                     color = group, shape = group), show.legend = F) +
      # geom_smooth(
...
ros_fig <- sjp.ros +
      ...
      geom_point(data = rename(fires_biomass, group = structure),
            aes(x = total_biomass, y = rate_of_spread_ms,color = group, shape = group)) +
...
ggsave("figures/ros-flame-consumed.png", height = 17, width = 8.5, units = "cm", dpi = 600)
