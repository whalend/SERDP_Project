...
Our experimental design was to conduct six burns, three using piled fuel and three using standing fuel, for each of five fuel loads (250 g, 500 g, 1000 g, 1500 g, 2000 g) spanning the range of cogongrass biomass observed from field measurements across Florida, USA. We ultimately conducted 32 fires in this experiment. One additional fire was done in the 1000g category due to mislabeling during biomass sorting and weighing.

For comparison points to the measurements from these experimental fires we also show data collected during a prescribed fire and data from a small number of experimental burns using the FABIO where fuels were both piled and standing. The prescribed fire was conducted in a long-leaf pine forest at Camp Blanding Joint Forces Training Center near Keystone Heights, FL, USA. The forest had prescribed fire applied regularly on an 18-36 month interval, promoting a herbaceous understory dominated by the bunch grass *Aristida stricta* (wiregrass). We successfully recorded temperatures and flame height at seven locations across the ~640 acre area.
...
We used ordinary least squares linear regression to model the response variables of average maximum temperature and time above 100 ºC at each probe height, the average flame height, and the rate of spread. We used beta regression to model the proportion of biomass consumed from each experimental fire. Fuel load (mass) and fuel structure (piled vs. standing) were used as explanatory variables in each model, and fuel load was entered as a continuous variable. We tested the main effects and interaction of fuel load and fuel structure. If the interaction did not have a strong effect we refit the model using only the main additive effects. Our main goal in this modeling was to assess the effect of fuel structure (piled vs. standing) on measures of fire intensity and behavior.

OLS models had the following form:
...
We limited the data for fires with both standing and piled (litter) to those where litter biomass was 500g (n=5). Our expectation is that the litter biomass increases fuel continuity and therefore affects the behavior of fires with relatively small standing fuel loads, i.e., 250g m^-2^ of standing fuel alone is different from a fire with an additional 500g ^-2^ of piled fuel (litter). In this example, we would expect that the 750g m^-2^ total (standing + piled) to have similar fire intensity to 750g m^-2^ of standing only fuel. 
...
**Estimates from fitted maximum temperature models**
```{r table all maxtemp coefs}

all_maxtemp_coefs <- rbind(coef50cm, coef25cm, coef0cm)
kable(all_maxtemp_coefs, digits = 4)
...
sabv_fig_labels <- data.frame(location = c("50cm","25cm","0cm"), pid = c("b","d","f"), total_biomass = c(200,200,200), s_abv100 = c(150,200,775))
...
kable(all_s_abv100_models, digits = 4)
...
kable(flame_ht_coefs, digits = 3, caption = "Model estimates of flame heights from FABIO piled vs. standing")

flame_ht_coefs <- flame_ht_coefs %>% 
      mutate(Variable = my_row_names) %>% 
      select(Variable, Estimate:ncol(flame_ht_coefs))

```

**Flame height model ANOVA table (Type II F-test)**
```{r flame ht model Anova type2}
...
ros_model_coef <- ros_model_coef %>% 
      ...
      select(Variable, 1:ncol(ros_model_coef))
...
rownames(ros_anova) <- c("Biomass", "Structure: standing", "Air temperature", "Biomass X Structure")

knitr::kable(ros_anova)

```

```{r ros-flame-consumed-fig, fig.height=8, fig.width=4}
...
cowplot::plot_grid(
      ...
      pct_consumed_fig +
            theme(legend.position = "none"),
      ncol = 1, labels = "auto")
ggsave("figures/ros-flame-consumed.png", height = 8, width = 4)

```

The confidence interval for the standing fuels in the rate of spread panel (b) don't extend to the 250g fuel load because only one fire at that fuel load had a measurable rate of spread. 
