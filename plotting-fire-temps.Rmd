...
time_abv100 <- d1 %>% filter(probe_type == "old") %>%
             group_by(location,fire_id, pct_green, standing_biomass, litter_biomass, remaining_biomass, dry_remaining_biomass, biomass_type) %>%
             filter(tempC>100) %>%
...
d2 <- left_join(max_temp,time_abv100)
d2 <- left_join(d2,time_abv300)
d2$s_abv300[is.na(d2$s_abv300)] <- 0
d2$s_abv100[is.na(d2$s_abv100)] <- 0
levels(d2$location)
...
```{r wiregrass vs cogongrass time above 100 ºC}
ggplot(filter(d2, between(standing_biomass,199,601)|standing_biomass==1000|standing_biomass==1500|standing_biomass==3000, litter_biomass==0), 
...
pfire_temps_abv100 <- pfire_temps %>% 
      ...
      summarise(time_abv100 = length(tempC),
...
ggplot(filter(pfire_data_summ, id != "fabio naturale"),
       aes(avg_flame_ht, time_abv100)) +
      geom_point(aes(shape = location, color = id)) +
      theme_bw()

ggplot(filter(pfire_data_summ, id != "fabio naturale"),
       aes(avg_fuel_ht, time_abv100)) +
      geom_point(aes(shape = location, color = id)) +
      theme_bw()
...
pfire_bar_data <- pfire_data_summ %>% 
      ...
      summarise(n_obs = length(location),
                avg_time_abv100 = mean(time_abv100),
                se_time_abv100 = sd(time_abv100)/sqrt(n_obs),
...
ggplot(pfire_bar_data,
       aes(location, avg_time_abv100)) +
      geom_bar(stat = "identity", fill = "grey60") +
      # scale_fill_continuous(low = "yellow", high = "red") +
      geom_errorbar(aes(ymin = avg_time_abv100 + se_time_abv100, 
                        ymax = avg_time_abv100 - se_time_abv100),
                    width = .1) +
      # facet_grid(location~.) +
      ylab("Seconds >100 ºC") +
...
```{r flame height sensur heat flux}
ggplot(pfire_data_summ,
