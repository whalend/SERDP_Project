...
* **`r n_distinct(plot_visit_data_noNA$years_since_fire)` different years since fire**
...
filter(plot_visit_data, is.na(inst_name))
plot_visit_data$inst_name[is.na(plot_visit_data$inst_name)] <- "Camp Blanding"
...
pct_lit_fire_mod <- lm(avg_pct_litter ~ years_since_fire + imcy_inv, data = quadrat10_summaries)
summary(pct_lit_fire_mod)
...
invaded_quadrats <- species10_data %>% 
      ...
      select(installation:quadrat_id, quadrat_invaded, years_since_fire, inst_name)
...
quadrat_species_richness <- species10_data %>%
      group_by(inst_name, years_since_fire, plot_id, quadrat_invaded, quadrat_id) %>%
      summarise(richness = n_distinct(scientific_name)) %>% 
      mutate(ln_richness = log(richness),
             max_richness = max(richness),
...
# filter(biomass_data, is.na(years_since_fire))
...
# filter(biomass_plot_summary, is.na(years_since_fire))$plot_id
...
ggplot(biomass_plot_summary,
       aes(imcy_inv, avg_wet_standing_gm2, color = imcy_inv)) +
...
litter_mass_model <- lm(avg_dry_litter_gm2 ~ years_since_fire + imcy_inv, data = biomass_plot_summary)
summary(litter_mass_model)
...
(shrub_saplings_fig <- ggplot(
      filter(subplot_plot_summary, total_stems_100m2>=0),
...
deer_dung_model <- lm(log1p(total_clusters1m) ~ years_since_fire*imcy_inv,
                      data = filter(dung_plot_data, species=="deer"))
summary(deer_dung_model)
...
tick_data %>% 
             filter(!is.na(years_since_fire)) %>% 
             group_by(inst_name, years_since_fire, visit_year) %>%
             summarise(tick_count = sum(tick_count)) %>% 
      ggplot(aes(inst_name, tick_count, fill = inst_name)) +
      # geom_point(position = "jitter") +
      geom_col() +
...
tick_data_life_stage_summary <- tick_data %>% 
      group_by(plot_id, visit_year, life_stage, years_since_fire, imcy_inv, inst_name, date) %>% 
      summarise(total_ticks = sum(tick_count)) %>% 
      ungroup(.)


tick_data_plot_summary <- tick_data %>% 
      group_by(installation, plot_id, visit_year, years_since_fire, imcy_inv, inst_name) %>% 
      summarise(total_ticks = sum(tick_count)) %>% 
      ungroup(.)
...
tick_data_sum <- left_join(
      tick_data_sum,
      ungroup(veg_cover_plot_data, -n_obs)
)
...
tick_data_sum <- left_join(tick_data_sum,
          select(plot_visit_data, plot_id, visit_year, visit_date))
...
tick_fire_model <- lm(log1p(total_ticks) ~ years_since_fire, data = tick_data_plot_summary)
...
Perhaps most useful for specific installations because that would avoid severe overlap of the points like we have here. 

```{r ticks-pine}

ggplot(tick_data_sum, aes(as.factor(years_since_fire), log1p(total_ticks))) +
      geom_point(aes(color = pct_pinus_dbh), size = 2, position = "jitter") +
...
host_fire_mod <- lm(log1p(total_clusters1m) ~ years_since_fire, data = tick_data_sum)
...
GGally::ggpairs(select(tick_data_sum, years_since_fire, avg_pct_litter, avg_pct_litter_moisture, avg_canopy_cover, pct_pinus_dbh))
...
model_data <- tick_data_sum %>% 
      filter(!(plot_id %in% extra_ticks),
             !is.na(last_fire_year)) %>% 
...
ggpairs(select(
      model_data, 
      total_ticks, years_since_fire, day_of_year, avg_dry_litter_gm2, avg_canopy_cover, total_clusters1m, total_dbh, pct_pinus_dbh, total_stems_100m2, lat, imcy_inv), aes(color = imcy_inv)) +
      # invasion_color +
      # invasion_fill +
      theme_bw() +
...
ggpairs(select(
      model_data, 
      total_ticks, years_since_fire, day_of_year, avg_dry_litter_gm2, avg_canopy_cover, total_clusters1m, clusters_1_2, total_dbh, pct_pinus_dbh, total_stems_100m2, lat)) +
      # invasion_color +
      # invasion_fill +
      theme_bw() +
      theme(panel.grid = element_blank())
...
glmer(total_ticks ~  (1|installation) + (1|plot_id), data = model_data, family = "poisson")
...
model_list <- list(
      ...
      host_mod <- glmer(total_clusters_1_2 ~ total_stems_100m2 + day_of_year + years_since_fire + f_visit_year +  (1|installation) + (1|plot_id), data = model_data, family = "poisson"),
      
      litter_mod <- lmer(avg_dry_litter_gm2 ~ years_since_fire + (1|installation) + (1|plot_id), data = model_data),
      ...
      canopy_mod <- lmer(avg_canopy_cover ~ years_since_fire + (1|installation) + (1|plot_id), data = model_data)
      
      # fire_mod <- lmer(years_since_fire ~ ycoord_lat + visit_year + (1|installation) + (1|plot_id), tick_data_sum)
      
)

psem.model <- as.psem(model_list)
psem.model.summary <- summary(psem.model)
...
library(sjPlot)
plot_model(tick_mod, type = "pred", terms = c("years_since_fire"), pred.type = "re", show.data = T)
...
cogon_biomass <- left_join(
      cogon_biomass,
      select(plot_visit_data, installation, visit_date, plot_id, years_since_fire, inst_name))
