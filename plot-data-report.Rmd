...
library(GGally)
ggpairs(select(tick_data_sum, years_since_fire, ycoord_lat, total_ticks, avg_dry_litter_gm2, avg_canopy_cover, total_clusters1m, pct_pinus_dbh, total_stems_100m2, imcy_inv), aes(color = imcy_inv)) +
      # invasion_color +
      # invasion_fill +
...
model_data <- tick_data_sum %>% 
      ...
      mutate()
filter(model_data, is.na(max_veg_cover))$plot_id
# summary(model_data)

glmer(total_ticks ~  (1|installation) + (1|plot_id), data = tick_data_sum, family = "poisson")

model_list <- list(
      
      tick_mod <- glmer(total_ticks ~ total_clusters1m + avg_dry_litter_gm2 + avg_canopy_cover + (1|installation) + (1|visit_year/plot_id), data = tick_data_sum, family = "poisson"),
      
      host_mod <- glmer(total_clusters1m ~ years_since_fire + avg_canopy_cover + (1|installation) + (1|visit_year/plot_id), data = tick_data_sum, family = "poisson"),
      
      litter_mod <- lmer(avg_dry_litter_gm2 ~ years_since_fire + as.factor(visit_year) + (1|installation) + (1|plot_id), data = tick_data_sum),
...
ggplot(filter(cogon_biomass, !is.na(inst_name)), 
       aes(years_since_fire, tiller_density)) +
      geom_point(aes(color = inst_name), size = 2) +
      scale_color_brewer(palette = "Dark2") +
      theme_classic() +
      xlab("Years since fire") +
