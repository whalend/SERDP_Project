...
# filter(plot_visit_data, is.na(n_fires))$plot_id
...
* **`r n_distinct(plot_visit_data_no$inst_name)` installations**
* **`r n_distinct(plot_visit_data_no$plot_id)` different plots, some multiple times**
* **`r length(plot_visit_data_no$plot_id)` plot visits (some revisits)**
...
plot_visit_data %>% 
      ggplot(aes(x = as.factor(last_fire_year))) +
...
(litter_depth_fig <- ggplot(quadrat10_summaries, aes(log(y_since_fire), avg_litter_depth_all)) +
      geom_point(aes(color = imcy_inv), size = 2) +
      geom_smooth(aes(color = imcy_inv, fill = imcy_inv), method = "lm", alpha = .2) +
...
summary(fx_group_data)
filter(fx_group_data, is.na(imcy_inv))$plot_id
...
invaded_quadrats <- species10_data %>% 
      ...
      select(plot_id:quadrat_id, quadrat_invaded, inst_name)
...
quadrat_species_richness <- species10_data %>%
      group_by(inst_name, plot_id, quadrat_invaded, quadrat_id) %>%
      summarise(richness = n_distinct(scientific_name)) %>% 
      mutate(ln_richness = log(richness),
             max_richness = max(richness),
...
(litter_mass_fig <- ggplot(biomass_plot_summary,
        aes(log(y_since_fire), avg_dry_litter_gm2, color = imcy_inv)) +  
      geom_point() +
...
ggplot(biomass_plot_summary,
        aes(log(frindex), avg_dry_litter_gm2, color = imcy_inv)) +  
      geom_point() +
      geom_smooth(aes(fill = imcy_inv), alpha = .2, method = "loess", linetype = "dashed", se = F) +
      geom_smooth(aes(fill = imcy_inv), alpha = .2, method = "lm", show.legend = F) +
      # stat_summary(fun.data = "mean_se", geom = "pointrange") +
...
(tick_fire_fig <- ggplot(filter(tick_data_sum,), 
      ...
      geom_smooth(se = T, method = "loess", alpha = .2) +
...
ggplot(tick_data_sum, 
       aes(log(fri15yr), log1p(total_ticks), fill = imcy_inv, color = imcy_inv)) +
...
ggplot(filter(tick_data_sum, !is.na(imcy_inv)), 
        aes(log(y_since_fire), log1p(total_clusters1m))) +
      geom_point(aes(color = imcy_inv), alpha = .5) +
      invasion_color +
      invasion_fill +
      # geom_smooth(aes(color = imcy_inv, fill = imcy_inv), method = "loess", se = F, alpha = .2, linetype = "dashed") +
      geom_smooth(aes(color = imcy_inv, fill = imcy_inv), method = "lm", se = T, alpha = .2) +
      theme_classic() +
      xlab(expression(paste("Years since fire (log)"))) +
      ylab("log( dung clusters + 1 )") +
      def_theme
...
tick_mod_inv_pairs <- ggpairs(select(
      model_data, 
      total_ticks, y_since_fire, day_of_year, avg_dry_litter_gm2, avg_pct_litter, avg_canopy_cover, total_clusters1m, total_dbh, pct_pinus_dbh, total_stems_100m2, imcy_inv), aes(color = imcy_inv),
       upper = list(
...
tick_mod_pairs <- ggpairs(select(
      model_data, 
      total_ticks, y_since_fire, day_of_year, avg_dry_litter_gm2, avg_canopy_cover, total_clusters1m, clusters_1_2, total_dbh, pct_pinus_dbh, total_stems_100m2, lat)) +
      theme_bw() +
      theme(panel.grid = element_blank())
...
m1 <- glmer(total_ticks ~ y_since_fire + (1|inst_name) + (1|plot_id), data = model_data, family = "poisson")
summary(m1)

m2 <- glmer.nb(total_ticks ~ y_since_fire + (1|inst_name) + (1|plot_id), data = model_data)
summary(m2)
...
psem.model.summary <- summary(mod)
