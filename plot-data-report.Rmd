...
```{r plot data manipulation, results='hide', eval=F}
# This is all done in a separate script now.
...
plot_coords <- select(plot_locations@data, name, installation = instal, xcoord_lon, ycoord_lat)
str(plot_coords)

plot_coords <- plot_coords %>% 
      mutate(name = as.character(name),
             installation= as.character(installation),
             plot_id = tolower(substr(name, start = nchar(name)-1, stop = nchar(name))),
             pid = paste(installation, plot_id, sep = " ")
      ) %>% 
      select(pid, xcoord_lon, ycoord_lat)
...
plot_visit_data <- left_join(plot_visit_data, plot_coords)
...
```{r summary of plots visited, fig.width=8, fig.height=5}
...
ggplot(plot_visit_data%>% 
       ...
       aes(installation_full_name, plot_ct, label=plot_ct)) +
      geom_col() +
      geom_text(nudge_y = .2) +
...
d <- sort(unique(species_data$sciname))
d
names(species_data)
unique(species_data$functional_group)
unique(species_data$functional_group__1)
filter(species_data, is.na(functional_group__1)) %>% 
      select(plot_id, date, veg_id)
...
species_data$pct_cover[species_data$installation=="avonpark" & species_data$date=="2018-05-31" & species_data$plot_id=="f1" & species_data$quadrat_id=="n10" & species_data$veg_id=="xyca"] <- 4
species_data$pct_cover[species_data$installation=="avonpark" & species_data$date=="2018-05-29" & species_data$plot_id=="e2" & species_data$quadrat_id=="e10" & species_data$veg_id=="dicl"] <- 4

species_data$pct_cover[species_data$installation=="eglin" & species_data$date=="2018-06-11" & species_data$plot_id=="a1" & species_data$quadrat_id=="e10" & species_data$veg_id=="an"] <- NA

species_data$pct_cover[species_data$installation=="tyndall" & species_data$date=="2018-06-25" & species_data$plot_id=="d2" & species_data$quadrat_id=="n10" & species_data$veg_id=="dicl"] <- NA
      
species_data$pct_cover[species_data$installation=="tyndall" & species_data$date=="2018-06-25" & species_data$plot_id=="d2" & species_data$quadrat_id=="n10" & species_data$veg_id=="piel"] <- NA   
species_data$pct_cover[species_data$installation=="jackson" & species_data$date=="2018-07-12" & species_data$plot_id=="h1" & species_data$quadrat_id=="w10" & species_data$veg_id=="saal5"] <- NA    
...
veg_cover_plot_data <- species_data %>%
      ...
      summarise(
            avg_veg_cover = mean(veg_cover, na.rm = T),
            n_obs = length(veg_cover),
            # max_veg_cover = max(veg_cover, na.rm = T),
            sd_veg_cover = sd(veg_cover, na.rm = T),
            se_veg_cover = sd_veg_cover / sqrt(length(veg_cover))) %>% 
      ungroup(.)
...
dung_plot_avg_data <- dung_data %>% 
      group_by(installation, pid, visit_year, years_since_fire, imcy_inv) %>% 
...
```{r tick life stage vs fire figure}

ggplot(tick_data %>% 
             filter(!is.na(imcy_inv), !is.na(life_stage)), 
       aes(years_since_fire, log1p(count), color = imcy_inv, fill = imcy_inv)) +
...
tick_data_life_stage_summary <- tick_data %>% 
      group_by(pid, visit_year, life_stage, years_since_fire, imcy_inv, installation_full_name, date) %>% 
...
model_list <- list(
      tick_mod <- glmer.nb(total_ticks ~ avg_clusters1m + avg_dry_litter_gm2 + avg_canopy_cover + (1|pid), data = tick_data_sum),
      host_mod <- glmer.nb(avg_clusters1m ~ years_since_fire + avg_canopy_cover + (1|pid), data = tick_data_sum),
      litter_mod <- lmer(avg_dry_litter_gm2 ~ years_since_fire + (1|pid), data = tick_data_sum),
