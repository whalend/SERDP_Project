...
subplot_data_import$date <- as.Date(as.character(subplot_data_import$date), format="%Y%m%d")
...
(shrub_saplings_fig <- ggplot(
      ...
      # stat_summary(fun.data = "mean_se", geom = "pointrange") +
      geom_point(aes(color = imcy_inv), alpha = .7, size = 2) +
...
(deer_dung_fig <- dung_plot_data %>% filter(species=="deer") %>% 
      ...
      invasion_color +
      # geom_smooth(aes(linetype = imcy_inv), se = F, alpha = .1) +
      geom_smooth(aes(fill = imcy_inv), se = T, method = "lm") +
      theme_classic() +
...
tick_data <- tick_data_import %>% 
      mutate(date = as.Date(as.character(tick_data_import$date), format = "%Y%m%d"),
             visit_year = lubridate::year(date),
...
ggplot(filter(tick_data_sum, !is.na(imcy_inv)), 
      ...
      # geom_smooth(aes(fill = imcy_inv), method = "loess", linetype = "dashed", se = F, alpha = .3, show.legend = F) +
      # facet_grid(imcy_inv~.) +
      theme_classic() +
      # theme_bw() +
...
tick_canopy_mod <- lm(log1p(total_ticks) ~ avg_canopy_cover + avg_pct_litter + avg_pct_litter_moisture, data = tick_data_sum)
summary(tick_canopy_mod)

sjPlot::plot_model(tick_canopy_mod, show.values = T, type = "diag")
sjPlot::plot_model(tick_canopy_mod, show.values = T, type = "std")
sjPlot::plot_model(tick_canopy_mod, show.values = T, type = "slope")
sjPlot::plot_model(tick_canopy_mod, show.values = T, type = "pred", terms = c("avg_pct_litter", "avg_pct_litter_moisture", "avg_canopy_cover"))
...
host_trees_mod <- lm(log1p(total_clusters1m) ~ total_dbh*imcy_inv, data = tick_data_sum)
