...
* **`r n_distinct(plot_visit_data$last_fire_year)` burn units**
* **`r n_distinct(plot_visit_data$pid)` plots**
...
quadrat_data <- filter(
      quadrat_data, 
      date > "2017-06-01",# discard early data collection at Camp Blanding
      plot_id != "cogan" # movie theater cogon plot
      )
...
quadrat_summaries <- quadrat_data %>%
      filter(quadrat_id == "n10" | quadrat_id == "e10" | 
                   quadrat_id=="s10" | quadrat_id=="w10") %>% 
      group_by(installation, plot_id, date) %>%
      summarise(
