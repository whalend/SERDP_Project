...
species_data <- filter(
      species_data, date>"2017-06-01",
      quadrat_id %in% c("e10","w10","n10","s10"))
...
# species_data$X__4
species_data <- select(
      species_data, Order:functional_group__1
)
...
filter(species_data, scientific_name=="NA NA")
...
species_data <- left_join(
      species_data, plot_visit_data, 
      by = c("installation","plot_id","date"="visit_date"))
...
veg_cover_data <- species_data %>% arrange(date) %>% 
      group_by(installation, date, plot_id, functional_group) %>% 
      summarise(
...
ggplot(veg_cover_data, aes(years_since_fire, avg_pct_cover)) +
...
species_richness <- species_data %>%
      group_by(installation, years_since_fire, imcy_inv, functional_group) %>%
...
ggplot(plot_fxgroups, 
       aes(years_since_fire, richness, color = functional_group)) +
      # stat_summary(aes(y = richness), fun.data = "mean_se", "pointrange") +
      geom_point(aes(color = functional_group), position = "jitter") +
...
ggplot(tree_data %>% 
             filter(!is.na(imcy_inv)),
       aes(imcy_inv, char, fill = imcy_inv)) +
      # stat_summary(fun.data = "mean_se", geom = "pointrange") +
      # geom_point(position = "jitter") +
      geom_bar(stat = "identity", position = "dodge") +
      invasion_color +
...
ggplot(subplot_data %>% 
             filter(!is.na(imcy_inv), stems_100m2<600), 
       aes(years_since_fire, stems_100m2)) +
      geom_point(aes(color = imcy_inv)) +
      invasion_color +
      # facet_grid(.~imcy_inv) +
      def_theme +
...
ggplot(biomass_data %>% 
             filter(!is.na(imcy_inv)),
       aes(years_since_fire, standing_fuel_mass_dry*16, color = imcy_inv)) +
      geom_point() +
      invasion_color +
      invasion_fill +
      # facet_grid(.~installation) +
...
dung_data_avg <- dung_data %>% 
      group_by(installation, plot_id, date, years_since_fire, species) %>% 
...
ggplot(dung_data %>% 
             filter(!is.na(imcy_inv)), 
       aes((years_since_fire), log1p(dung1m))) +
      # geom_bar(aes(fill = imcy_inv), stat = "identity", position = "dodge") +
...
tick_data$visit_year <- lubridate::year(tick_data$date)
...
unique(tolower(tick_data$species))
