...
ggplot(plot_data, aes(installation_full_name)) +
...
cc_inv <- left_join(canopy_cover_summary, plot_data)

ggplot(cc_inv, aes(as.factor(years_since_fire), avg_canopy_cover)) +
...
ggplot(tree_data %>%
             group_by(full_names, labels, years_since_fire, imcy_inv),
       aes(as.factor(years_since_fire), dbh, color = labels)) +
      geom_point(position = "jitter") +
      scale_fill_brewer(palette = "Dark2") +
      facet_grid(full_names~imcy_inv) +
      theme_bw() +
      xlab("Years since fire") +
...
dung_data_avg <- left_join(dung_data_avg, plot_data)
...
ggplot(tick_data %>% 
             group_by(Date, full_names) %>% 
             summarise(abundance = sum(count)),
       aes(Date, abundance)) +
      # geom_point(aes(color = installation), size = 2) +
      geom_bar(aes(fill = full_names), stat = "identity") +
...
tick_data_sum <- tick_data %>% 
      group_by(installation, plot_id, full_names, years_since_fire) %>% 
...
d1 <- left_join(d1, select(avg_species_cover, full_names:max_veg_cover))
