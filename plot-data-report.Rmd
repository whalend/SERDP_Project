...
avg_biomass_data <- biomass_data %>% 
      ...
      mutate(standing_gm2 = fuel_mass_wet*16,
...
ggplot(d1, aes(avg_litter_gm2, log1p(total_ticks))) +
      ...
      geom_smooth(method = "lm") +
      theme_bw() +
      xlab(expression(paste("Average Litter Biomass (g/", m^{2}, ")"))) +
      ylab("Tick Abundance (ln)") +
      def_theme
```
...
cogon_data <- read_csv("~/Dropbox (UF)/SERDP-Project/data/cogon_data.csv")
