...
Due to the low number of cogongrass-invaded plots on DoD installations we identified an additional 28(??) sites across Florida that had cogongrass invasions ≥ .1 ha(??) and located at least partially within forested areas. A plot was also established in a nearby uninvaded area at XX of these sites.
...
dung_data$species2 <- ifelse(!dung_data$species %in% c("cottontail", "cow", "deer", "turkey"), 
                             "other", dung_data$species)
...
dung_plot_data <- dung_data %>% 
      # filter(species == "deer") %>% 
      group_by(plot_id, visit_year, imcy_inv, species2, inst_name) %>% 
      summarise(
...
dung_plot_data <- dung_plot_data %>% 
  ungroup(.) %>% 
  select(plot_id:inst_name, species2, total_clusters1m) %>% 
  pivot_wider(names_from = species2, values_from = total_clusters1m, values_fill = 0) %>% 
  mutate(total_clusters1m = cottontail  + cow + other + deer + turkey)
summary(dung_plot_data)

dung_plot_data_summary <- left_join(
      dung_plot_data,
      plot_visit_data
)
...
(dung_species_fig <- ggplot(filter(dung_data, !is.na(dung1m), visit_year<2019), 
                            aes(installation, y=(dung1m))) +
...
# tick_data$inst_name
a <- tick_data %>% filter(is.na(inst_name))
# unique(a$location)
a <- tick_data %>% distinct(installation, inst_name) %>% 
  filter(!is.na(inst_name))

tick_data <- left_join(tick_data %>% select(-inst_name), a, 
                       by = "installation") 

a <- tick_data %>% 
  distinct(installation, plot_id, location, visit_date, visit_year, collection_method) %>% 
  # filter(location != "extra") %>% # for plot level effort
  mutate(trap_effort = ifelse(collection_method=="trap", 1, 0)) %>% 
  group_by(installation,  
           plot_id,
           visit_year
           ) %>% 
  summarise(trap_effort = sum(trap_effort, na.rm = TRUE))
# summary(a)
a %>% filter(trap_effort == 0)
# write_csv(a, "data/plot_trapping_effort.csv")
# write_csv(a, "data/installation_trapping_effort.csv")
...
```{r summarize-tick-abundance-data, results='hide', eval=TRUE}
...
ticks_per_installation <- tick_data %>% 
      # filter(location != "extra") %>%
      group_by(installation, inst_name) %>%
...
tick_data_sum <- left_join(
      ...
      ungroup(dung_plot_data_summary) %>% 
        select(plot_id:total_clusters1m)
)
# summary(tick_data_sum)
...
write_csv(tick_data_sum, "data/processed_data/all_plotlevel_data.csv")
...
ticks_climate_15yr <- tick_data_sum %>% 
      ...
      pivot_longer(., avg_15yr_srad..W.m.2.:cv_15yr_fire_days)
...
ticks_climate_30yr <- tick_data_sum %>% 
      ...
      pivot_longer(., avg_30yr_srad..W.m.2.:cv_30yr_fire_days)
