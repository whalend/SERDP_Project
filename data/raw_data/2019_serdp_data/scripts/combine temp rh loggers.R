...
pd = "data/raw_data/2019_serdp_data/2019-logger-sampling/" # parent directory

file_list <- list.files("data/raw_data/2019_serdp_data/2019-logger-sampling/", pattern = "csv")
...
for(f in file_list){
  ...
  n = n[,c(1:4)]
  names(n) <- col_names
  n$logger_id = paste(stringr::str_sub(f, start = 1, end = 27))
  df1 <- rbind(df1, n)
}
...
write_csv(df1, "data/processed_data/2019-secondary-logger-sampling.csv")
